# Water Programming Blog Post Catalog

The table below contains posts from the [Water Programming Blog](https://waterprogramming.wordpress.com/) written by Reed group members and collaborates over the last several years. Search by author, keyword, or title to filter the table. 

<div style="position:relative; height: 100%;">
  <iframe src="post_catalog.html" 
  style="width: 100%; height: 100%;"
  frameborder="0" allowfullscreen>
  </iframe>
</div>



### Updating this table:
Current Reed Group members looking to update the table should read the instructions listed in the [reedgroup/waterprogramming_archive/README](https://github.com/reedgroup/waterprogramming_archive).  

*Note that this is only accessible to current members of the Reed Group GitHub organization.*